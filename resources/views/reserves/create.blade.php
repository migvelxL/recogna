...
<x-app-layout class="bg-black">
    <?php if (!empty(session('message'))) { ?>
        <div id="message" class="transition ease-in-out bg-{{session('color')}}-100 border border-{{session('color')}}-400 text-red-{{session('color')}} px-4 py-3 rounded relative" role="alert">
            ...
            <div class="bg-white overflow-hidden shadow-sm p-5 justify-items-center sm:rounded-lg">
                <div class="flex items-center justify-center h-full">
                    <div class="col-md-8">
                        ...
                        <div class="">
                            <form method="POST" class="mt-4 grid grid-cols-2 gap-4" action="{{ route('reserve.store') }}">
                                @csrf
                                ...
                                <input type="hidden" class="pointer-events-none p-2 border-slate-300 rounded" name="space_id" value="{{ $maquina->id }}">

                                <div class="form-group flex flex-column">
                                    <label for="space_name">Local</label>
                                    <input type="text" class="pointer-events-none p-2 border-slate-300 rounded" name="space_name" value="{{ $maquina->name }}">
                                </div>
                                <div class="form-group flex flex-column">
                                    <label for="name">Data da reserva: (formato: mês / dia / ano)</label>
                                    <input id="allocation" autofocus type="date" class="form-control @error('data') is-invalid @enderror p-2 border-slate-300 rounded" name="allocation" required autocomplete="allocation" autofocus>
                                    @error('data')
